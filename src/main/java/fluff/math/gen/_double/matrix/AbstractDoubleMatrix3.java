...
public abstract class AbstractDoubleMatrix3<V extends AbstractDoubleMatrix3<V>> implements IDoubleMatrix<V> {
	
	public abstract double get(int i, int j, int k);
	
	public abstract V set(int i, int j, int k, double val);
	...
	public V multiply(V mat) {
		double [][][] result = new double [getDepth()][getRows()][mat.getColumns()];
        for (int i = 0; i < getDepth(); i++) {
            for (int j = 0; j < getRows(); j++) {
	...
	public boolean equals(Object obj) {
		if (obj instanceof AbstractDoubleMatrix3 mat) {
	...
	public int hashCode() {
		...
		for (int i = 0; i < getDepth(); i++) {
			for (int j = 0; j < getRows(); j++) {
				for (int k = 0; k < getColumns(); k++) {
					bits = 31L * bits + Double.doubleToLongBits(get(i, j, k));
				}
			}
		}
	...
	protected boolean isSameSize(AbstractDoubleMatrix3<V> mat) {
